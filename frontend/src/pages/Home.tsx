 ...
 * - 「进入 AI 助手」fixed 悬浮在视口底部；离开场景序列后淡出，
 *   避免遮挡下方卡片区（那里已有「开始对话」按钮）
 */
const Home: React.FC = () => {
  ...
  return (
    <div className="bg-[#f6f5f0]">
      {/* 沉浸式场景：4 张整屏图，滚动逐屏切换 */}
      <ImmersiveScenes onImmersiveChange={setImmersive} />

      {/* 固定 CTA：场景序列内悬浮可见，离开后淡出 */}
      <div
        className={`fixed bottom-6 left-1/2 z-40 -translate-x-1/2 transition-opacity duration-500 ${
          immersive ? 'opacity-100' : 'pointer-events-none opacity-0'
        }`}
      >
        <Link
          to="/agent"
